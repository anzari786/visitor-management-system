...
export const getUsers = async (req: Request, res: Response) => {
   ...
   const { users, meta } = await listUsers({
      ...
      authProvider,
      passwordSetupPending:
         passwordSetupPending === undefined
            ? undefined
...
export const postPasswordSetup = async (req: Request, res: Response) => {
   const { id } = req.validatedParams as UserIdParams;

   const result = await invitePasswordSetup(id);

   return res.status(200).json({
      success: true,
      message: 'Password setup invitation sent',
      data: {
